...
namespace Flow.IO
{
    public class DeleteFiles : PipelineAction
    {
        ...
        private static async Task<IValueSource> Handler(DeleteFiles that, IExecutionContext context, PayloadCollection paths)
        { return await Handler(that, context, paths.Cast<FilePath>().ToArray()); }

        private static async Task<IValueSource> Handler(DeleteFiles that, IExecutionContext context, params FilePath[] paths)
        { return await Handler(that, context, paths.Select(p => p.Path).ToArray()); }

        private static async Task<IValueSource> Handler(DeleteFiles that, IExecutionContext context, IEnumerable<string> paths)
        {
